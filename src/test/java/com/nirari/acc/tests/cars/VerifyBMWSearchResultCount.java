...
public class VerifyBMWSearchResultCount extends BaseTest {
    ...
    public void verifyBMWSearchResultCount() {
        ...
        and("Verify the number of results displayed");
            TMSearchResults results = visitor.gotoPage(TMSearchResults.class);
            int resultCount = Integer.parseInt(results.resultCountLabel.getText().split(" ")[1]
                    .replace(",", ""));
            assertThat(resultCount, equalTo(4169)); //Could use less strict assertion, such as n > 0 or n > x
    }
}
